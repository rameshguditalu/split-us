...
const Home = () => {
    ...
    const [groups, setGroups] = useState([{ id: '' }]);
    const [expenses, setExpenses] = useState([{ id: '' }]);
    const [expensePerMonth, setExpensePerMonth] = useState([{ month: 0, count: 0 }]);
        ...
        async function fetchHomeStats() {
            try {
                ...
                await getDocs(q).then(async (groupsData) => {
                    console.log(groupsData.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
                    const groups = groupsData.docs.map((doc) => ({ ...doc.data(), id: doc.id }))
                    setGroups(groups);
                    ...
                    for (let i = 0; i < expensesData.docs.length; i++) {
                        if (
                            ...
                            monthMap.set(
                                expensesData.docs[i].data().createdAt.toDate().getMonth(),
                                monthMap.get(expensesData?.docs[i].data().createdAt.toDate().getMonth()) ? + 1
                            );
                        } else {
                            monthMap.set(
                                expensesData.docs[i].data().createdAt.toDate().getMonth(),
                        ...
                        setExpensesCategoryMap((prevState) => ({
                            ...
                            [expensesData.docs[i].data().expenseCategory]:
                                expensesCategoryMap[expensesData.docs[i].data().expenseCategory] + 1,
                        }));
                    }
        ...
        if (props.active && props.payload && props.payload.length) {
            return (
                <div className="custom-tooltip">
                    <p className="label">{`${monthName[props.label]} : ${props.payload[0].value}`}</p>
                </div>
            );
        }
